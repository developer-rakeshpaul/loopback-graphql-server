...
function mapProperty(
  ...
  if (scalar) {
    ...
    if (property.enum) {
      // enum has a dedicated type but no input type is required
      types[typeName] = {
        generated: false,
        name: typeName,
  ...
  if (
    ...
    if (union.length > 1) {
      ...
      propertyType.settings.anonymous &&
        ...
        .then(() => {
          ...
          _.forEach(acceptingParams, (param, name) => {
            if (
              ...
              Object.keys(args[name]).length > 0
            ) {
          ...
          if (isManyRelation(rel.type) === true) {
            return connectionFromPromisedArray(
              findRelatedMany(rel, obj, params, context),
              args,
              rel.modelTo
            );
          } else {
